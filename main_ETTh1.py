...
# Set dataset path and file name for the ETTh1 dataset
...
# Define the TimeSeriesDataset class for PyTorch
...
# Preprocessing function for ETTh1 dataset
def preprocess_data(data, qubit_no=20):
    print(f"Preprocessing ETTh1 data with qubit no {qubit_no}...")
    ...
    # Process the ETTh1 data using 'HUFL' column
    if 'HUFL' in data.columns:
        print("Processing ETTh1 data using 'HUFL' column...")
        data['HUFL'] = scaler.fit_transform(data['HUFL'].values.reshape(-1, 1))
        X = np.array([data['HUFL'].values[i:i + qubit_no] for i in range(len(data) - qubit_no)])
        y_true = np.array([1 if val > 0.8 else 0 for val in data['HUFL'][qubit_no:]])
    else:
...
def load_datasets():
    ...
    data_ETTh1 = pd.read_csv(file_path)
    print(f"Loaded ETTh1 dataset with columns: {data_ETTh1.columns}")
    return {'ETTh1': data_ETTh1}
...
def print_comparison_table(results, y_test_dict):
    print(f"\n{'Method':<25} {'MCC':<8} {'F1':<8} {'Accuracy':<10} {'TP Rate':<10} {'TN Rate':<10} {'NAB Score':<10} {'Complexity':<12} {'Time (s)':<12} {'Size (MB)':<10}")
