...
# Load dataset
def load_datasets():
    print("Loading WAADM dataset...")
    file_path = dataset_path + dataset_name
    data_WAADM = pd.read_csv(file_path)
    print(f"Loaded WAADM dataset with columns: {data_WAADM.columns}")
    return {'WAADM': data_WAADM}
...
def classical_methods(X_train, y_train, X_test, y_test):
    models = {
        ...
        'Robust Covariance (EllipticEnvelope)': EllipticEnvelope(),
...
def run_comparison(datasets, window_size=20):
    ...
    for name, data in datasets.items():
        ...
        results[name] = {'quantum_metrics': quantum_metrics, 'classical_accuracies': classical_accuracies}
...
def print_comparison_table(results):
    for dataset_name, res in results.items():
        ...
        print(f"{'Method':<25} {'MCC':<8} {'F1':<8} {'Accuracy':<10} {'TP Rate':<10} {'TN Rate':<10} {'PR AUC':<8} {'ROC AUC':<8}")
